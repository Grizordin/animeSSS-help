...
let held=new Map(),timerId=-1,lastHeldDelay=0;
window.setTimeout=(fn,ms,...args)=>{if([10000,3000,1200,400].includes(ms)){lastHeldDelay=ms;const id=timerId--;held.set(id,()=>fn(...args));return id;}return nativeSetTimeout(fn,ms,...args);};
...
cfg.autoOpenTarget=100;startAutoOpen();held.clear();
const packsPage=document.querySelector('.packs-page');
packsPage.dataset.packMotion='off';c=card('no-animation');
const beforeDelay=clicked;autoClickBestCard(c);
check(clicked===beforeDelay&&autoBusy&&!autoPendingChoice,'no-animation choice waits without counting or clicking');
check(held.size===1&&lastHeldDelay===1200&&autoStatusEl.textContent.includes('1,2'),'1200 ms pause and status');
let chooseAfterDelay=[...held.values()][0];held.clear();chooseAfterDelay();
check(clicked===beforeDelay+1&&!!autoPendingChoice,'choice dispatched after pause');
getActiveRow().remove();await flush();held.clear();
c=card('no-animation-stopped');autoClickBestCard(c);
chooseAfterDelay=[...held.values()][0];held.clear();stopAutoOpen();startAutoOpen();chooseAfterDelay();
check(clicked===beforeDelay+1&&!autoPendingChoice,'stop/restart invalidates delayed choice');
c=card('no-animation-replaced');autoClickBestCard(c);
chooseAfterDelay=[...held.values()][0];held.clear();card('replacement');chooseAfterDelay();
check(clicked===beforeDelay+1&&!autoPendingChoice&&!autoBusy,'replaced card cannot be selected by stale callback');
rare=true;c=card('rare-without-animation');autoClickBestCard(c);
check(held.size===1&&lastHeldDelay===3000&&autoStatusEl.textContent.includes('3 сек'),'rare pause remains single 3-second pause');
stopAutoOpen();held.clear();rare=false;packsPage.dataset.packMotion='on';
const noMotion=document.createElement('input');noMotion.type='checkbox';noMotion.id='disable_pack_animation';noMotion.checked=true;packsPage.prepend(noMotion);
startAutoOpen();c=card('checkbox-off');autoClickBestCard(c);
check(held.size===1&&lastHeldDelay===1200&&clicked===beforeDelay+1,'checked site option also delays selection');
stopAutoOpen();held.clear();noMotion.remove();
const originalMatchMedia=window.matchMedia;
window.matchMedia=q=>q==='(prefers-reduced-motion: reduce)'?{matches:true}:originalMatchMedia.call(window,q);
startAutoOpen();c=card('reduced-motion');autoClickBestCard(c);
check(held.size===1&&lastHeldDelay===1200&&clicked===beforeDelay+1,'system reduced motion also waits 1200 ms');
stopAutoOpen();held.clear();window.matchMedia=originalMatchMedia;
